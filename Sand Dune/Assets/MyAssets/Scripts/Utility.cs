...
public class Utility {
	...
	/// <param name ="curve">The curve on which the ratio will be computed.</param>
	/// <param name="ratio">The value to compute on the curve. Usually a float between 0 and 1.</param>
	/// <param name="multiplier">The maximum value that should be returned. Return value will be relative to multiplier.</param>
	/// <returns>Returns the Y-value of the curve at the given X-value as a float multiplied by the constant multiplier.</returns>
	public static float EvaluateCurve(AnimationCurve curve, float ratio, float multiplier) {
		return curve.Evaluate(ratio) * multiplier;
	}
}
